...
import { getPkgManager, type PackageManager } from "../utils/get-pkg-manager.js";
...
export async function createAppCommand(projectName: string, options: CreateAppOptions) {
	const questions = [];
			...
			choices: [
				{
					...
					description: "Build a serverless backend with AWS Lambda, API Gateway",
				},
				{
		...
		questions.push({
			...
			choices: [
				...
				},
				{
					...
					description: "Backend with DynamoDB using Dynamoose ORM, full CRUD example.",
				},
				{
					title: "CRUD Backend (Mongoose)",
					value: "with-mongoose",
					description: "Backend with MongoDB using Mongoose ORM, full CRUD example.",
				},
			],
		});
	}
	...
	if (!options.importAlias) {
		...
		questions.push({
			...
			validate: (value: string) =>
				importAliasPattern.test(value) ? true : "Import alias must follow the pattern <prefix>/*",
		});
	}

	if (!options.useNpm && !options.usePnpm && !options.useYarn && !options.skipInstall) {
		questions.push({
			type: "confirm",
			name: "skipInstall",
 ...
 /_____|  \\__,_| | .__/  |_| /_/\\_\\
	...
	const packageManager: PackageManager = config.packageManager || getPkgManager();
...
export async function createProject(configs: CreateProjectConfigs): Promise<void> {
	const { disableGit, projectPath, skipInstall, example, examplePath, packageManager, template } = configs;
	...
	if (example) {
		...
		if (repoUrl) {
			...
			if (!repoInfo) {
				console.error(`Found invalid GitHub URL: ${picocolors.red(`"${example}"`)}. Please fix the URL and try again.`);
				process.exit(1);
			}
			...
			if (!found) {
				console.error(
					`Could not locate an example named ${picocolors.red(`"${example}"`)}. It could be due to the following:\n`,
					`1. Your spelling of example ${picocolors.red(`"${example}"`)} might be incorrect.\n`,
	...
	if (example) {
		try {
			if (repoInfo) {
				...
				console.log(`Downloading files from repo ${picocolors.cyan(example)}. This might take a moment.`);
				console.log();
				await retry(() => downloadAndExtractRepo(root, repoInfo2), {
					retries: 3,
				});
			} else {
				console.log(`Downloading files for example ${picocolors.cyan(example)}. This might take a moment.`);
				console.log();
	...
	console.log(`${picocolors.green("Success!")} Created ${appName} at ${projectPath}`);

	if (hasPackageJson) {
		console.log("Inside that directory, you can run several commands:");
		console.log();
		console.log(picocolors.cyan(`  ${packageManager} ${useYarn ? "" : "run "}dev`));
		console.log("    Starts the development server.");
		console.log();
		console.log(picocolors.cyan(`  ${packageManager} ${useYarn ? "" : "run "}build`));
